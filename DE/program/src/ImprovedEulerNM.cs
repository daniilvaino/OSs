...
// Type: ComputationalPractice.src.ImprovedEulerNM
...
{
    internal class ImprovedEulerNM : NumericalMethod
    {

        public ImprovedEulerNM(double x0, double y0, double X, int N)
        {
            this.Name = "Improved Euler";
            if (x0 > X)
        ...
        public override Dictionary<double, double> GetSample(double x0, double y0, double X, int N)
        {
            ...
            for (int index = 1; index <= N; ++index)
            {
                ...
                double num3 = Problem.f(x0 + (double)index * num1, dictionary[x0 + (double)(index - 1) * num1] + num1 * num2);
                dictionary.Add(x0 + (double)index * num1, dictionary[x0 + (double)(index - 1) * num1] + num1 * ((num2 + num3) / 2.0));
            }
            return dictionary;
        }
